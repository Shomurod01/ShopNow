...
import { SearchIcon, HeartIcon, CartIcon, ChevronDownIcon, MenuIcon } from './Icons'
          ...
          <div className="flex items-center h-16 gap-4">
                      ...
                      <div className="absolute right-0 mt-2 w-56 bg-white text-gray-800 rounded-xl shadow-2xl py-2 z-20 border border-gray-100">
                        ...
                        {!isAdmin && (
                          <>
                            ...
                            <Link to="/wishlist" onClick={() => setDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-gray-50">My Wishlist</Link>
                          </>
                        )}
                        {isAdmin && (
                          <Link to="/admin" onClick={() => setDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50 font-semibold">Admin Dashboard</Link>
                        )}
            ...
            <div className="flex items-center gap-3 md:hidden ml-auto">
              ...
              <button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-300">
                <MenuIcon className="h-6 w-6" open={menuOpen} />
              </button>
            </div>
          </div>
        </div>
        ...
        {!isAdmin && (
          <div className="bg-slate-800 border-t border-slate-700 hidden md:block">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-6 h-9 text-xs overflow-x-auto">
              <Link to="/products" className="text-gray-200 hover:text-amber-400 whitespace-nowrap font-bold transition-colors">All Products</Link>
              {CATEGORY_NAMES.map(cat => (
                <Link key={cat} to={`/products?category=${encodeURIComponent(cat)}`} className="text-gray-400 hover:text-amber-400 whitespace-nowrap transition-colors">{cat}</Link>
              ))}
